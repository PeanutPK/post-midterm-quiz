...
class Table:
    ...
    def update_row(self, primary_attribute, primary_attribute_value,
        ...
        """
        This method updates the current value of update_attribute
        to update_value
        For example,
        ...
        for data in self.table:
            if data[primary_attribute] == primary_attribute_value:
                data[update_attribute] = update_value
...
my_db = DB()
my_table = Table("Movie", movies)
print(my_table.table)
movie_filter_commedy = my_table.filter(lambda x: x['Genre'] == 'Comedy')
money = []
for item in movie_filter_commedy.table:
    money.append(float(item['Worldwide Gross']))
print(movie_filter_commedy.table)
print(money)
print("average value of ‘Worldwide Gross’ for ‘Comedy’ movies: ", end='')
print(sum(money) / len(money))

movie_filter_drama = my_table.filter(lambda x: x['Genre'] == 'Drama')
min_movie_drama = movie_filter_drama.aggregate(min, 'Audience score %')
print("minimum ‘Audience score %’ for ‘Drama’ movies: ", end='')
print(min_movie_drama)
